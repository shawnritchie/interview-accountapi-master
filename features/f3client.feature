...
#    Then we expect a http status code: Not Found
  ...
  Scenario: get first page from list api
    ...
    And a random accountId
    And a valid account has been registered
    When I send "GET" request to "/v1/organisation/accounts/" for page 1 with a page size of 20
    Then we expect a valid response
  ...
  Scenario: get first page from list api
    ...
    And a random accountId
    And a valid account has been registered
    And I send "GET" request to "/v1/organisation/accounts/" for page 1 with a page size of 20
